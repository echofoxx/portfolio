...
## v0.7.6 Travel routes and exports

| Method | Route | Purpose |
|---|---|---|
| GET | `/travel` | Filtered travel dashboard and request/report inventory |
| GET | `/travel/requests/{id}` | Traveler-level approval detail and provenance |
| GET | `/travel/reports/{id}` | Full trip report, matching, outcomes, links, and audit |
| POST | `/travel/reports/{id}/match` | Confirm or clear a request match |
| POST | `/travel/reports/{id}/review` | Record report review status and comments |
| POST | `/travel/report-items/{id}/promote` | Promote an outcome to a canonical portfolio record or dismiss it |
| GET | `/travel/engagements/{id}` | Consolidated engagement detail |
| GET | `/exports/travel-requests.csv` | Accessible travel-request CSV |
| GET | `/exports/trip-reports.csv` | Accessible trip-report CSV |

All routes require an authenticated session. Write routes require CSRF validation and one of the configured portfolio-management roles. Organization and sensitivity scoping is applied server-side.

## v0.7.7 visual-intelligence query parameters

### Travel geographic filter

`GET /travel?location=<canonical location>` filters the authorized request/report population to the canonical destination generated by the local governed location registry. Existing `division`, `determination`, `q`, `start`, and `end` filters remain supported.

### Financial flow filters

`GET /financials` accepts optional `category`, `division`, and `view` parameters. `view=actual` selects records with actual cost; `view=remaining` selects records whose approved budget exceeds actual cost. These parameters support Investment Flow drill-through and do not change source data.

The dashboard map and Sankey data are embedded as escaped same-origin JSON in authenticated HTML. No public visualization API or external data transfer is introduced.
